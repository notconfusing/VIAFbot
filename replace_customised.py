# -*- coding: utf-8 -*-
#customisation by Max Klein are commented with the text '#customised' following the customisation
"""
            ...
            if self.acceptall and new_text != original_text:
                if not self.articles:
                    ...
                    try:
                        ...
                        raise pywikibot.EditConflict #customised
                        pywikibot.output(u'Skipping %s because of edit conflict'
                                         % (page.title(),))
                    except pywikibot.SpamfilterError, e:
                        raise pywikibot.SpamfilterError #customised
                        pywikibot.output(
                            u'Cannot change %s because of blacklist entry %s'
                            % (page.title(), e.url))
                    except pywikibot.PageNotSaved, error:
                        raise pywikibot.PageNotSaved #customised
                        pywikibot.output(u'Error putting page: %s'
                                         % (error.args,))
                    except pywikibot.LockedPage:
                        raise pywikibot.LockedPage #customised
